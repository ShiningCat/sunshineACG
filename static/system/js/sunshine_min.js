...
var zan = {
    ...
    topFixed: function() {
        var b = jQuery('#zan-header');
        var c = jQuery('body');
        var d = b.find('input[type="checkbox"]');
        var e = window.localStorage;
        e.setItem('ifFixed', 'float');
        if (!e.getItem('ifFixed')) {
            e.setItem('ifFixed', 'float')
        } else {
            if (e.getItem('ifFixed') == 'fixed') {
        ...
        d.on('ifChecked', function(a) {
            b.addClass('navbar-fixed-top');
            c.addClass('nav-fixed');
            e.setItem('ifFixed', 'fixed')
        }).on('ifUnchecked', function(a) {
            b.removeClass('navbar-fixed-top');
            c.removeClass('nav-fixed');
            e.setItem('ifFixed', 'float')
        })
    ...
    dropDown: function() {
        var a = jQuery('.nav.navbar-nav li');
    ...
    panelToggle: function() {
        var b = jQuery('.panel-toggle');
        b.data('toggle', true);
        b.click(function() {
            var a = jQuery(this);
            if (a.data('toggle')) {
                a.removeClass('fa-chevron-circle-up').addClass('fa-chevron-circle-down');
                a.parents('div.panel').addClass('toggled');
                a.data('toggle', false)
            } else {
                a.removeClass('fa-chevron-circle-down').addClass('fa-chevron-circle-up');
                a.parents('div.panel').removeClass('toggled');
                a.data('toggle', true)
            }
        })
    },
    panelClose: function() {
        var b = jQuery('.panel-remove');
        b.click(function() {
            var a = jQuery(this);
            a.parents('.panel').toggle(300)
        })
    },
    btnLoading: function() {
        var a = jQuery('#load-more');
        var b = a.attr('load-data');
        a.click(function() {
            a.addClass('disabled');
    ...
    setImgHeight: function() {
        var g = jQuery(".content-article p").find("img");
        g.each(function() {
            var a = jQuery(this);
            var b = a.attr('width');
            var c = a.attr('height');
    ...
    archivesNum: function() {
        jQuery('#archives .panel-body').each(function() {
            var a = jQuery(this).find('p').size();
            var b = jQuery(this).parent().siblings().find("a");
            var c = b.text();
            b.html(c + ' <small>(' + a + '篇文章)</small>')
        })
    },
    lazyload: function() {
        jQuery("#sidebar img.lazy").lazyload({
            threshold: 800
        });
        jQuery("#smilelink img.lazy").lazyload({
            threshold: 800
        });
    ...
    ajaxCommentsReply: function() {
        ...
        $submit.attr('disabled', false), $body = (window.opera) ? (document.compatMode == "CSS1Compat" ? $('html') : $('body')) : $('html,body'), comm_array = [];
        comm_array.push('');
        ...
        $(document).on("submit", "#commentform", function() {
            ...
            $.ajax({
                ...
                error: function(a) {
                    $('#loading').hide();
                    $("#error").slideDown().html(a.responseText);
                    setTimeout(function() {
        ...
        addComment = {
            moveForm: function(a, b, c, d, f) {
                ...
                f ? (t.I('comment').value = comm_array[f], $new_sucs = $('#success_' + f), $new_sucs.hide(), $new_comm = $('#new-comm-' + f), $cancel.text(cancel_edit)) : $cancel.text(cancel_text);
                t.respondId = c;
                d = d || false;
                zan.lazyload();
    ...
    ajaxCommentsPage: function() {
        ...
        $('#comment-nav a').live('click', function(e) {
            ...
            $.ajax({
                ...
                dataType: "html",
                success: function(a) {
                    result = $(a).find('.commentlist');
                    nextlink = $(a).find('#comment-nav');
